...
public class AddToCart extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!Util.isInteger(Pid)) {
            ...
            SellersProduct product = (SellersProduct) s.get(SellersProduct.class, Integer.valueOf(Pid));
                ...
                if (user != null) { //addproduct db m=user found in session
                    ...
                    if (c1.list().isEmpty()) {// no product in same pid
                        ...
                        if (Integer.parseInt(qty) <= product.getQty()) {
                            ...
                            responseObject.addProperty("message", "Product addto cart Success");
                        } else {
                        ...
                        if (newQty <= Integer.parseInt(qty)) {
                            ...
                            tr.commit();
                            responseObject.addProperty("status", Boolean.TRUE);
                            responseObject.addProperty("message", "Product cart Update Success");
                        } else {
                            responseObject.addProperty("message", "OOPS! Insufficient Product quantity");
                        }
                    ...
                    if (ses.getAttribute("sessionCart") == null) {
                        if (Integer.parseInt(qty) <= product.getQty()) {
                            ...
                            responseObject.addProperty("message", "Product add to cart Success");
                        } else {
                            responseObject.addProperty("message", "OOPS! Insufficient Product quantity");
                        }
